...
import { Appointment, PatientAppoinment } from "@/lib/types";

export function AppointmentCard({
  appointment,
  userRole,
  refetchAppointments,
}:{appointment:Appointment|PatientAppoinment,userRole:string,refetchAppointments?:()=>void}) {
  const [open, setOpen] = useState(false);
  const [action, setAction] = useState<string|null>(null); // 'cancel', 'notes', 'video', or 'complete'
  ...
  const otherParty =
  userRole === "DOCTOR"
    ? (appointment as Appointment).patient
    : (appointment as PatientAppoinment).doctor;
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <div className="flex items-center mt-2 text-sm text-muted-foreground">
                  ...
                  <span>{formatDateTime(appointment.startTime.toString())}</span>
                </div>
              ...
              <Badge
                ...
                className={
                  appointment.status === "COMPLETED"
                    ? "bg-emerald-300/20 border-emerald-900/30 text-emerald-400"
                    : appointment.status === "CANCELLED"
                    ? "bg-red-300/20 border-red-900/30 text-red-400"
                    : "bg-amber-300/20 border-amber-900/30 text-amber-400"
                }
              >
                {appointment.status}
              </Badge>

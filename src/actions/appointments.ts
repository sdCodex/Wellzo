...
import { AvailableTimeSlotsResult, SlotItem,Get_Doctors } from "@/lib/types";
 ...
 * Get doctor by ID
 */
export async function getDoctorById(doctorId: string):Promise<{doctor:Get_Doctors}> {
  try {
...
export async function getAvailableTimeSlots(doctorId: string):Promise<AvailableTimeSlotsResult>{
  try {
    ...
    const availableSlotsByDay: Record<string, SlotItem[]> = {};
...
export async function bookAppointment(formData:FormData) {
  ...
  try {
    ...
    const patientDescription = formData.get("description")?.toString();
...
export async function generateVideoToken(formData:FormData) {
  ...
  try {
    ...
    const videoSessionId=appointment.videoSessionId?.toString();

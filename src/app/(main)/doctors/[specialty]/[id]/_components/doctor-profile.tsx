...
import { AvailableTimeSlotsResult, Get_Doctors, SlotItem } from "@/lib/types";



export function DoctorProfile({ doctor, availableDays }:{doctor:Get_Doctors,availableDays:AvailableTimeSlotsResult}) {
  const [showBooking, setShowBooking] = useState(false);
  const [selectedSlot, setSelectedSlot] = useState<SlotItem | null>(null);
  const router = useRouter();

  // Calculate total available slots
  const totalSlots = availableDays?.days?.reduce(
          ...
          <Card className="border-emerald-900/20">
            <CardContent className="pt-6">
              <div className="flex flex-col items-center text-center">
                ...
                <Badge
                  variant="outline"
                  className=" border-emerald-900/30 text-emerald-400 mb-4"
                >
                  {doctor.specialty}
                </Badge>

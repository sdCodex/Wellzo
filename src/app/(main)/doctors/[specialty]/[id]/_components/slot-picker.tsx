...
import { AvailableTimeSlotsResult, SlotItem } from "@/lib/types";

export function SlotPicker({ availableDays, onSelectSlot }:{availableDays:AvailableTimeSlotsResult, onSelectSlot:(slot:SlotItem)=>void}) {
  const [selectedSlot, setSelectedSlot] = useState<SlotItem|null>(null);

  // Find first day with slots as default tab
  const firstDayWithSlots =
    availableDays.days.find((day) => day.slots.length > 0)?.date || availableDays.days[0]?.date;
        ...
        <TabsList className="w-full justify-start overflow-x-auto">
          {availableDays.days.map((day) => (
            ...
            >
              ...
              {day.slots.length > 0 && (
                <div className="ml-2 bg-emerald-300/30 text-emerald-400 text-xs px-2 py-1 rounded">
                  {day.slots.length}
                </div>
              )}
            </TabsTrigger>
